...
export async function testarPush(userId: string): Promise<{ ok: boolean; etapa: 'suporte' | 'assinar' | 'enviar'; localOk: boolean; detalhe?: any }> {
  ...
  try {
    const reg = await navigator.serviceWorker.ready
    await reg.showNotification('Teste do AXIS (local)', { body: 'Se você vê isso, o aparelho exibe notificações.', badge: '/axis-badge.png', icon: '/transparente.png', tag: 'teste-local' })
    localOk = true
  } catch {}
...
export async function diagnosticoPush(userId: string): Promise<ResultadoDiag> {
  ...
  let localOk = false
  try {
    if (reg) { await reg.showNotification('Diagnóstico AXIS (local)', { body: 'Se você VÊ isto, o aparelho exibe notificações.', badge: '/axis-badge.png', icon: '/transparente.png', tag: 'diag-local' }); localOk = true }
  } catch {}
