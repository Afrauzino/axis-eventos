...
export default function BotaoConfig() {
  ...
  return (
    <>
      ...
      {/* ADMINISTRAÇÃO — drawer lateral no MESMO padrão da gaveta (cabeçalho + lista com emoji) */}
      {aberto && (chrome.railGrupos?.length ?? 0) > 0 && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 600, display: 'flex' }}
          ...
          <div style={{ height: '100dvh', maxWidth: 340, width: '86%', background: 'white', display: 'flex', flexDirection: 'column', boxShadow: '2px 0 18px rgba(0,0,0,0.25)' }}>
            {/* Cabeçalho igual ao da gaveta */}
            <div style={{ padding: '22px 20px 16px', background: 'var(--primary-dark)', display: 'flex', alignItems: 'center', gap: 12, flexShrink: 0 }}>
              <button onClick={() => setAberto(false)} aria-label="Voltar"
                style={{ background: 'rgba(255,255,255,0.15)', border: 'none', borderRadius: 8, width: 34, height: 34, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontFamily: 'inherit', flexShrink: 0 }}>
                <span className="icon">arrow_back</span>
              </button>
              <div>
                <p style={{ fontSize: 16, fontWeight: 800, color: 'white', lineHeight: 1.1 }}>Administração</p>
                <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.75)', marginTop: 2 }}>Configurações do sistema</p>
              </div>
            </div>
            {/* Lista única, agrupada — rows com emoji + nome (padrão da gaveta) */}
            <div style={{ flex: 1, overflowY: 'auto', padding: '8px 0' }}>
              {chrome.railGrupos!.map((gr, gi) => (
                <div key={gi}>
                  <p style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.04em', textTransform: 'uppercase', color: 'var(--muted)', padding: '12px 20px 4px' }}>{gr.titulo}</p>
                  {gr.itens.map((it, ii) => (
                    <button key={ii} onClick={() => { setAberto(false); it.onClick() }}
                      style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 12, padding: '12px 20px', cursor: 'pointer', fontFamily: 'inherit', textAlign: 'left',
                        background: it.ativo ? 'var(--primary-light)' : 'none', border: 'none', borderLeft: it.ativo ? '3px solid var(--primary)' : '3px solid transparent' }}>
                      <span style={{ fontSize: 20, width: 24, textAlign: 'center', flexShrink: 0 }}>{it.emoji ?? '•'}</span>
                      <span style={{ flex: 1, fontSize: 14, fontWeight: it.ativo ? 700 : 500, color: it.ativo ? 'var(--primary-dark)' : 'var(--text)' }}>{it.label}</span>
                    </button>
                  ))}
                </div>
              ))}
