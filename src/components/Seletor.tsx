...
export default function Seletor({ value, onChange, opcoes, titulo = 'Selecione', placeholder = 'Selecione...', disabled, inline, sheet }: Props) {
  ...
  return (
    <>
      <button type="button" disabled={disabled} onClick={() => !disabled && setAberto(true)}
        ...
        <span style={{ fontSize: 14, color: atual ? 'var(--text)' : 'var(--muted)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
          {atual ? `${atual.emoji ? atual.emoji + ' ' : ''}${atual.label}` : placeholder}
        </span>
        <span className="icon icon-sm" style={{ color: 'var(--primary)', flexShrink: 0 }}>expand_more</span>
      </button>
