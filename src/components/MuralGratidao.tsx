...
export default function MuralGratidao({ eventoId, profile, fundo, onEditar }: { eventoId?: string; profile: Profile; fundo?: BlocoFundo; onEditar?: (aspecto: number) => void }) {
  ...
  async function toggleCurtida(postId: string) {
    ...
    if (jaCurtiu) {
      ...
      else { const autor = posts.find(p => p.id === postId)?.user_id; if (autor) notificarRegra('mural_reacao', { user_ids: [autor], title: `${profile.full_name ?? 'Alguém'} curtiu seu recado`, body: '', url: '/?ir=mural' }) }
    }
  }
