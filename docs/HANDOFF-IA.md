...
### 🐛 Reenvio de cadastro recusado ficava PRESO — ✅ conserto em sql/77 (mexe no seu trg_trava_privilegio)
Anderson testou (13/07): recusou um cadastro, a pessoa corrigiu e reenviou, mas ficou presa na tela "Cadastro não aprovado" e o admin nunca recebeu pra reaprovar. **Causa:** o seu `trg_trava_privilegio` (sql/73) reverte `role_status` pra QUALQUER não-admin — inclusive o reenvio legítimo (`reenviar_meu_cadastro` faz rejected→pending, mas o trigger volta pra rejected). **Fix (sql/77, Anderson vai rodar):** o trigger agora permite SÓ a transição do próprio cadastro `rejected→pending`; todo o resto (is_admin, user_role, outros pulos de status) continua travado. Sem auto-promoção. Também liguei a notificação de reenvio pro admin (Pending.tsx → notificarRegra insc_nova). **Obs.:** esse mesmo trigger pode estar por trás do "cadastro por código perde dados" que diagnostiquei antes — vale conferir se ele não atrapalha o vínculo inicial.
